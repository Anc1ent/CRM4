...
<div style="float:left;padding-left:13px; ">EDIT SMS</div>
<div id="closeSMSWindow" onclick="$('#editemailblock').fadeOut(500); $('#EditSMSContainer').fadeOut(500);" style="float:right; padding-right:15px; font-size:13px; cursor:pointer; padding:3px; padding-left:10px; padding-right:10px; background-color:rgba(255,255,255,0.2);">close</div>
<div style="clear:both;"></div>
</div>  <div style=" padding:20px;  background-color:#ffffff;">
            <form method="POST" action="<?=base_url()?>admin/main/addchsms/">
              <input type="hidden" name="Eid" id="Emid2" value="<?=$data->id?>">
              <div style="float:left; width:40%;">
                <div style="font-size:11px; " class="PformLabel">SMS name</div>
                <div><input type="text" style="width:100%;" name="Sname" value="<?=$data->name?>" /></div>

                <!--<div style="font-size:11px; margin-top:10px;" class="PformLabel">SMS subject</div>
              ...
              <div><textarea id="textareaAR2<?=$hash?>"  style="font-size:12px; height:400px;"><?=stripslashes($data->text)?></textarea></div>
              <div style="clear:both;"></div>
              ...
              <input type="button" class="buttonBlue" style="width:auto; cursor:pointer; text-align:center; margin-top:20px; padding:10px; border-radius:10px; border-radius:10px;" onclick="UpdateSMS($(this).parent(), '<?=$data->id?>', '<?=$hash?>'); $('#EditSMSContainer').fadeOut(500);" class="iButton"   value=" Save SMS "/>
 
              <input type="button" id="saveTemporary" class="buttonGreen" style="width:auto; display:none; cursor:pointer; margin-top:20px; padding:10px; margin-left:20px; border-radius:10px;" onclick="$('#Emid2').val('0'); UpdateSMS($(this).parent(), '0', '<?=$hash?>'); $('#EditSMSContainer').fadeOut(500);" class="iButton buttonGreen"  value=" Save as temporary shablon to once send "/>
              ...
              <input type="button" onclick="$('#EID2').append($('<option>', { value: <?=$data->id?>,  text: '' })); $('#EID2 option[value=<?=$data->id?>]').prop('selected', 'selected'); UpdateSMS($(this).parent(), '<?=$data->id?>', '<?=$hash?>'); $('#EditSMSContainer').fadeOut(500);" class="iButton buttonGreen" style="width:auto; margin-top:20px;"  value=" Save SMS "/>
                <?php
                } ?>
            </form>
