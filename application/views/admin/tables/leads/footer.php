   ...
   <div class="ibutton buttonBlue" id="showMoreButton" style="padding:5px; border-radius:10px; font-weight:bold; padding-left:30px; padding-right:30px;  " onclick="addEmptyLead('<?=base_url()?>');">ADD EMPTY LEAD</div>
   <?php if($LIMIT < $countQ){ ?>
    <div onclick="getMoreQuotes('<?=base_url()?>', <?=$LIMIT?>, <?=$countQ?>);" style="padding:5px; border-radius:10px; font-weight:bold; padding-left:30px; padding-right:30px;  margin-left:10px;" class="ibutton buttonBlue" id="showMoreButton" >GET MORE QUOTES</div>
    <?php } ?>
