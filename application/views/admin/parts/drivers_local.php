...
<div>
<div id="RouteHistoryButton" onclick="show_drivers_tableF('<?=base_url()?>', $(this), '<?=$quote->id?>')" style="float:left; background-color:#ffffff; color:#333333; text-align:center; padding:10px; padding-left:20px; padding-right:20px; font-size:14px; cursor:pointer; font-weight:bold;  ">ROUTE HISTORY</div>
<div id="LocalCompaniesButton" onclick="show_drivers_tableLC('<?=base_url()?>', $(this), '<?=$quote->id?>' '<?=$sorting?>')"  style="float:left; background-color:#80ACD1; color:#ffffff; text-align:center; padding:10px; padding-left:20px; padding-right:20px; font-size:14px; cursor:pointer; font-weight:bold;  ">LOCAL COMPANIES</div>
...
<table class="insideTableWhiteGrey" border="0" style="width:100%;" cellpadding="0" cellspacing="0">
		<tr class="head" style="background-color:#80ACD1; color:#ffffff;">
				...
				<td style="text-align:left; "><div onclick="show_drivers_mass_launch_block('<?=base_url()?>', $(this), '<?=$qid?>');" style="cursor: pointer; margin-top:-3px; font-size:12px; border-radius:10px; cursor: pointer;  margin-left: 0px;  margin-right: 0px;   padding: 2px;   display: inline-block;  padding-right: 10px;  padding-left: 10px; font-size:10px;" class="iRedButton buttonOther">MASS LAUNCH</div></td>
				<td style="text-align:center; width:20px;">ID</td>
                <td style="padding-left:20px; width:150px;">Company Name<div class="sorting" onclick="sort_toggle($(this)); show_drivers_tableLC('<?=base_url()?>', $(this), '<?=$quote->id?>', '<?=$sorting?>')" style="cursor: pointer; position: relative; font-size:9px; background-color:#0077b3; text-align:center; width:45px"><?php if($sorting=="Origin"){echo "Destination";}else{echo "Origin";}?></div></td>
				<td style="width:120px;">Phone</td>
				<td style="width:70px;">Email</td>
                <td style="width:70px;"></td>

				<!--<td >Price per mile</td>-->
				<!--<td text-align:left;     width: 70px;">CHECKED</td>-->
        ...
        /*
			...
			*/
			?>
		<tr style="cursor:pointer;" class="contentRow DriversRow">
			<td style="text-align:center; width:5px; font-size:12px;"> <input type="checkbox" style="margin-left:10px; width:15px; margin-top:0px; cursor:pointer;" id="<?=$item->id?>" class="DSelectedItem"/></td>
			...
			<td onclick="event.stopPropagation();" style=" width:100px; font-size:11px; text-align:left; padding-left:10px; padding-right:10px;">
	         	...
	         	<?php $countNotices = 0; ?>
	           	 ...
	           	 <div title="Notices" onclick="show_quote_notice('<?=base_url()?>', '<?=(($item->id)*(-1))?>', $(this));" style="cursor:pointer; margin:3px auto; font-size:9px; border-radius:7px; display:inline-block; padding:3px; padding-left:6px; padding-right:6px; <?php if($item->countNotices > 0) { echo "background-color:#ff5050; color:#ffffff;"; }else{ echo "background-color:#dddddd; color:#000000;";  } ?> "><?=$item->countNotices?></div>
	        
      			 <div onclick="event.stopPropagation(); show_driver_launch_block('<?=base_url()?>', $(this), '<?=$item->id?>', '<?=$qid?>');" style="margin-top:2px; font-size:12px; border-radius:10px; cursor: pointer;  margin-left: 0px;  margin-right: 0px;   padding: 5px; border:none !important;  display: inline-block;  margin-left:0px; padding-right: 10px;  padding-left: 10px;" class="iRedButton buttonGreen">LAUNCH DITAILS</div>
    		</td>
			<td style="text-align:center; width:20px; font-size:12px;"><?=$item->id?></td>
			 
			<td style="font-size:12px; padding-left:20px; width:150px;" title="<?=$item->name?>"><?php echo substr($item->name, 0, 100)."...";?> <?php if($item->coment != "") { ?><span  title="<?=$item->coment?>" style="cursor:pointer; padding:3px; display:inline-block; background-color:#ff5050; color:#ffffff; margin-left:5px; padding-left:10px; padding-right:10px;">i</span><?php } ?></td>
			...
			<!--date('d/m/y', strtotime($item->atDate));
			date('h:i A', strtotime($item->atDate));-->
            <td style="font-size:12px;width:35px;"><img class="drv_id_<?=$item->id?>" src=<?php if(is_null($item->drivers_id)) {echo("\"".base_url()."img/favorites_empty.png\"");}else{ echo("\"".base_url()."img/favorites_full.png\"");}?> onclick="favorites('<?=base_url()?>', $(this), '<?=$item->id?>', '<?=$item->drivers_id?>')" /></td>
