...
<div onclick="if($('#fullMapBlock').is(':visible')){ $('#fullMapBlock').slideUp(300); $('#fullClientBlock').slideUp(300); }else{ $('#fullMapBlock').slideDown(300); $('#fullClientBlock').slideDown(300); } " style="font-size:13px; text-transform:uppercase; padding:15px; cursor:pointer;  background-color:#7A8CA2; font-weight:bold; color:#ffffff; height:15px;">
		...
		<div id="fullMapBlock" style="height:250px;  border-bottom: solid 2px #7A8CA2; background-size:cover; <?php if($carpic != "-"){ echo "background-image:url('".$carpic."');"; }else{ echo "background-image:url('".base_url()."img/noimage.jpg');"; } ?> background-size:cover; ">
...
<div style="float:left; width:calc(55% - 5px);">		
<div onclick="if($('#fullMapBlock').is(':visible')){ $('#fullMapBlock').slideUp(300); $('#fullClientBlock').slideUp(300); }else{ $('#fullMapBlock').slideDown(300); $('#fullClientBlock').slideDown(300); } " style="font-size:13px; text-transform:uppercase; padding:15px; cursor:pointer; padding-bottom:16px;  background-color:#7A8CA2; font-weight:bold; color:#ffffff; height:15px;">
	...
	<div id="fullClientBlock" style=" height:251px; margin-left:5px; background-color:#ffffff; border-bottom: solid 2px #7A8CA2;">
	<div style="float:left; width:55%;">
		<div style="padding:15px; padding-top:10px;  padding-left:20px; padding-right:20px;">
				...
				<div style="margin-top:10px; font-size:14px; margin-bottom:10px; margin-left:20px; color:#75879F;">
						...
						<div style="margin-top:0px;">Total Mileage: <span style="padding-left:3px; color:#91CBE5; font-weight:bold;" id="Fdistance" contenteditable="true" onchange="$('#FsaveButton').fadeIn(500); $('#FcencelButton').fadeIn(500);"><?=$quote->distance?></span></div>
						<script type="text/javascript">
							$( function() {
						...
						<div style="margin-top:15px; margin-left:-20px;">Ready Date: <input class="buttonBlue" id="datepickerLSd" style="margin-left:3px; cursor:pointer; display:inline-block; padding:5px; width:57px; border-radius:10px; background-color:#1970B0; padding-left:10px; padding-right:10px; color:#91CBE5; font-size:15px;" onchange="$('#FsaveButton').fadeIn(500); $('#FcencelButton').fadeIn(500);" value="<?=date('d/m/y', strtotime($quote->arriveDate))?>" /></div>
				</div>
          ...
          <?php 
			...
			<div style="clear:both;"></div>
				...
				<div style="font-size:16px; color:#75879F; margin-top:10px;">
					...
					<span style="padding-left:3px; color:#91CBE5; font-size:20px;" on>$</span><span id="FtotalPrice" contenteditable="true" onchange="$('#FsaveButton').fadeIn(500); $('#FcencelButton').fadeIn(500); recountTBprices(0);" style=" color:#91CBE5; font-size:20px;"><?=($quote->price + $quote->deposit)?></span> 
					</div>
					<div style="clear:both;"></div>
				</div>
				...
				<div onclick="show_bookit('<?=base_url()?>', $(this), '<?=$quote->id?>');" style="margin-left:3px; cursor:pointer; font-size:16px; display:block; padding:5px; border-radius:10px; background-color:rgb(0,255,128); text-align:center; padding-left:10px; padding-right:10px; font-weight:bold; color:#005696">BOOKIT NOW!</div>

				<div onclick="show_send_stat('<?=base_url()?>', $(this), '<?=$quote->id?>');" style="margin-left:3px; margin-top:5px; cursor:pointer; font-size:16px; display:block; padding:5px; border-radius:10px; background-color:#ff5050; text-align:center; padding-left:10px; padding-right:10px; font-weight:bold; color:#ffffff">LAUNCH DITAILS</div>
				</div>
			</div>
		</div>	
	</div>
...
<div style="border-left:none; position:absolute; top:0px; left:0px; z-index:2; width:100%; display:none; border-top:none; background-color:#ffffff;  height:250px; border-bottom:solid 3px #1970B0; " id="driversF" >
       <div style="position:relative;">
       	<table class="insideTableWhiteGrey" border="0" style="width:100%; position:absolute; top:0px; left:0px;" cellpadding="0" cellspacing="0">
			<tr class="head" style="background-color:#1970B0; color:#ffffff;">
				<!--td style="text-align:center; width:70px;">ID</td-->
				<td style="padding-left:20px; width:250px;">Company Name</td>
				<td style="width:170px;">Phone</td>
				<td style="width:120px;">Email</td>
				<td >Price per mile</td>
				<td style="text-align:left;     width: 130px;">DISPATCH</td>
				<td style="text-align:center;     width: 120px;">DATE CHECKED</td>
			</tr>
		</table>
