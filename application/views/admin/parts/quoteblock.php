...
<div onclick="if($('#fullMapBlock').is(':visible')){ $('#fullMapBlock').slideUp(300); $('#fullClientBlock').slideUp(300); }else{ $('#fullMapBlock').slideDown(300); $('#fullClientBlock').slideDown(300); } " style="font-size:13px; text-transform:uppercase; padding:15px; padding-bottom:16px; cursor:pointer;  background-color:#7A8CA2; font-weight:bold; height:15px; color:#ffffff; ">
				...
				<div style="margin-top:10px; font-size:14px; margin-bottom:10px; margin-left:20px; color:#75879F;">
						...
						<script type="text/javascript">
							$( function() {
							    ...
							    $("#datepickerLSd" ).datepicker("option", "minDate", "<?=date('d/m/y', strtotime($quote->arriveDate))?>" );
		...
		<div id="FcencelButton" onclick="activateQuote('<?=base_url()?>','<?=$quote->id?>', 0);" style="position:absolute; display:none; bottom:10px; left:-60px; background-color:#FF5050; border-radius:50px; color:#ffffff; padding:5px; font-size:14px; cursor:pointer; padding-left:10px; padding-right:10px;">
          ...
          <?php 
			...
			<div style="clear:both;"></div>
				...
				<div onclick="show_bookit('<?=base_url()?>', $(this), '<?=$quote->id?>');" class="buttonGreen" style="margin-left:3px; cursor:pointer; font-size:16px; display:block; padding:5px; border-radius:10px; background-color:rgb(0,255,128); text-align:center; padding-left:10px; padding-right:10px; font-weight:bold; color:#005696">BOOKIT NOW!</div>
